    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                Counter1[p] = emissionModel[trueDistance] * self.beliefs[p]
            self.beliefs = Counter1
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        Counter1 = util.Counter()
        if noisyDistance == None:
            Counter1[self.getJailPosition()] = 1.0
            self.beliefs = Counter1
        else:
            Counter1 = util.Counter()
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                Counter1[p] += emissionModel[trueDistance]
            self.beliefs = Counter1
    ...
    def elapseTime(self, gameState):
        ...
        while(n< len(self.particles)):
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[n]))
            self.particles[n] = util.sample(newPosDist)
            n += 1
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        jailghosts = [i for i in range(self.numGhosts) if noisyDistances[i] == None]
        Counter1 = util.Counter()
    ...
    def elapseTime(self, gameState):
        ...
        ghost, to obtain the dstributions over new positions for that
