...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        a = len(actions)
        while (a > 0):
            qVal.append(self.getQValue(state, actions[len(actions) - a]))
            a-=1
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            ...
            if qVal > mQVal or mAction is None:
                mQVal = qVal
                mAction = action
        return mAction
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        prim = (1 - self.alpha) * self.getQValue(state, action)
        ...
        sec = self.alpha * sample
        self.q_values[(state, action)] = prim + sec
